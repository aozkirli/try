...
switch k
    case 1
        output_file = strcat('DoG_S_r',num2str(repeat_number),'.mat');
    case 2
        output_file = strcat('DoG_R_r',num2str(repeat_number),'.mat');
    case 3
        output_file = strcat('DoG_SR_r',num2str(repeat_number),'.mat');        
    case 4
        output_file = strcat('DoG_AR_r',num2str(repeat_number),'.mat');
end
...
switch k
    case {1,2}
        mu_temp = deg2rad([ 0; 30; 10 ]); % [ aStim; peakLocStim; ySigma ];
        std_temp = deg2rad([ 1; 5; 2 ]); % population sigma
    case 3
        mu_temp = deg2rad([ 0; 30; 0; 30; 10 ]); % [ aStim; peakLocStim; aResp; peakLocResp; ySigma ];
        std_temp = deg2rad([ 1; 5; 1; 5; 2 ]); % population sigma
    case 4
        mu_temp = [ .5; -5; 30; .5; 5; 30; 10 ]; % [ betaRepulsion; aRepulsion; peakLocRepulsion; betaAttraction; aAttraction; peakLocAttraction; ySigma ];;
        mu_temp([2:3 5:7]) = deg2rad(mu_temp([2:3 5:7]));
        std_temp = [ .15; 1; 5; .15; 1; 5; 2 ]; % population sigma
        std_temp([2:3 5:7]) = deg2rad(std_temp([2:3 5:7]));
end
...
switch k
    ...
    case 4
        sample_matrix = reshape( samples, n_params, numel(samples)/n_params );
        sample_matrix( [2:3 5:7], : ) = rad2deg(sample_matrix( [2:3 5:7], : ));
        samples = reshape( sample_matrix, length(curr_sample), n_samples );
end
...
sample_matrix_lin = sample_matrix( [1 4], 1:end-2 );
sample_matrix_rad = sample_matrix( [2:3 5:7], 1:end-2 );

% prior
switch k
    case 4
        mu_prior = sample_matrix( : , end-1 );
        mu_vector_lin = repmat( mu_prior([1 4]), n_subjs, 1 );
        mu_vector_rad = repmat( mu_prior([2:3 5:7]), n_subjs, 1 );

        std_prior = sample_matrix( :, end );
        std_vector_lin = repmat( std_prior([1 4]), n_subjs, 1 );
        std_vector_rad = repmat( std_prior([2:3 5:7]), n_subjs, 1 );

        log_prior = sum(log(normpdf( sample_matrix_lin(:), mu_vector_lin, std_vector_lin ))) ...
            + sum(log(circ_vmpdf( sample_matrix_rad(:), mu_vector_rad, 1./std_vector_rad.^2 )));
end
...
switch k
    ...
    case 4
        gammaMode = [ .075; .5; 2.5; .075; .5; 2.5; 1 ]; % [ .5; 2.5; .5; 2.5; 1 ]; % sd(y)/2
        gammaMode([2:3 5:7]) = deg2rad(gammaMode([2:3 5:7]));
end
...
log_hyper_prior = sum(log(gampdf( std_prior, gammaShape, gammaScale ))) ...
    + sum(log(normpdf( mu_prior([1 4]), .5, .5 )));
...
switch k
    ...
    case 4
        flagImplausible = sum( sample_matrix( 2, 1:end-1 ) >= 0 ) > 0 || sum( sample_matrix( 5, 1:end-1 ) <= 0 ) > 0 || ...
            sum(sum( sample_matrix( [3 6], 1:end-1 ) <= pi/36 )) > 0 || sum(sum( sample_matrix( [3 6], 1:end-1 ) >= pi/2 )) > 0 || ...
            sum( sample_matrix( 7, 1:end-1 ) <= eps ) > 0;
end
if flagImplausible
    log_prior_all = -inf;
else
...
switch k
    ...
    case 4
        ...
        betaRepulsion = params_subj(1);
        aRepulsion = params_subj(2);
        peakLocRepulsion = params_subj(3);
        betaAttraction = params_subj(4);
        aAttraction = params_subj(5);
        peakLocAttraction = params_subj(6);
        ySigma = params_subj(7);

        xRepulsion = data(:,1) + betaRepulsion * data(:,end);
        xAttraction = data(:,1) + betaAttraction * data(:,end);

        log_likeli_subj = sum(log(circ_vmpdf( data(:,4), DoG( xRepulsion, aRepulsion, peakLocRepulsion ) + DoG( xAttraction, aAttraction, peakLocAttraction ), 1./ySigma.^2 )));
