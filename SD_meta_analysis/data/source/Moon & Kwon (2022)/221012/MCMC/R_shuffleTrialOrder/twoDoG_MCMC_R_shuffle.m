...
for k = 1:2
    for s = subjs
        ...
        % Label trial number
        currdata(:,end+1) = 1:length(currdata);
...
for k = 1:2
    parfor repeat_number = 1:4
        [samples, prop_cov] = oneDoG_MCMC( dataIndv, repeat_number, k );
    end
end
...
%% :: FUNCTION :: DoG_MCMC
function [samples, prop_cov] = oneDoG_MCMC( dataIndv, repeat_number, k )

switch k
    case 1
        output_file = strcat('oneDoG_R_shuffle_uncorrected_r',num2str(repeat_number),'.mat');
    case 2
        output_file = strcat('oneDoG_R_shuffle_corrected_r',num2str(repeat_number),'.mat');
end
...
mu_temp = [ 0; 30; 10 ]; % [ aStim; peakLocStim; aResp; peakLocResp; ySigma ];
mu_temp = repmat( mu_temp, n_subjs + 1, 1 ); % 8 subjects + population mu
...
%
log_prior_curr = cal_log_prior(curr_sample, n_params, n_subjs);
...
for i = 1:n_samples*thinning
    ...
    log_prior_prop = cal_log_prior(proposal_sample, n_params, n_subjs);
...
function log_prior_all = cal_log_prior(sample, n_params, n_subjs)
...
% hyper-prior
gammaMode = deg2rad( [ .5; 2.5; 1 ] ); % [ .5; 2.5; .5; 2.5; 1 ]; % sd(y)/2
...
% finalize
if sum(sum( sample_matrix( 2, 1:end-1 ) <= pi/36 )) > 0 || sum(sum( sample_matrix( 2, 1:end-1 ) >= pi/2 )) > 0 ...
        || sum(sum( sample_matrix( 3, 1:end-1 ) <= eps )) > 0
    log_prior_all = -inf;
else
...
for s = 1:n_subjs
    ...
    log_likeli_all(s) = cal_log_lik_subj( params, data );
...
function log_likeli_subj = cal_log_lik_subj( params_subj, data )
...
log_likeli_subj = sum(log(circ_vmpdf( data(:,4), DoG( x, a, peakLoc ), 1./ySigma.^2 )));
