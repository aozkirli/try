...
for i = 1:2
    ...
    cd(currentDir);
    load(['Houborg et al Exp ' num2str(i) ' (2023).mat']);
    % remove outlier subjects as in the main paper
    tbl                 = tbl_subset(tbl,'propout','<0.25','ccorr','>.4');
    ...
    tmp.rt          = tbl.rt;
    tmp.block       = tbl.block;
    tmp.delta       = tbl.d_between; % target delta
    tmp.trial       = tbl.trial;

    tbl                 = tmp;
    tbl_name            = 'Houborg et al. (2023)';
    ...
    fname         = ['Houborg_et_al_2023_Exp', num2str(i)];
