function  [dataPath, logPath] = estimation_block_practice(cfg, Gparams)

log     = [];
log.cfg = cfg;

% Set up data and log paths
dataPath = [Gparams.DataPath, 'data_S', num2str(Gparams.subject_number), '_Practice_Estimation_Block_', num2str(cfg.curBlock)];
while(exist([dataPath, '.mat'], 'file'))
    dataPath = [dataPath, '_1'];
end
dataPath = [dataPath, '.mat'];

logPath = [Gparams.LogPath, 'log_S', num2str(Gparams.subject_number),'_Practice_Estimation_Block_', num2str(cfg.curBlock)];
