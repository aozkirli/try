function [B, LL] = JV10_fit_djm (X, T, NT)
% JV10_FIT (X, T, NT)
...
%   --> www.paulbays.com
%
% djm: also fit target mu (as 5th member of B)...not sure how in this scheme! 
% (keep non-target mu at zero so number of parameters doesn't change with number of items) 
...
for i=1:length(K)
    for j=1:length(N)
        for k=1:length(U)
            [b ll] = JV10_function_djm(X,T,NT,[K(i) 1-N(j)-U(k) N(j) U(k)]);
            if (ll>LL)
                LL = ll;
                B = b;
            end
