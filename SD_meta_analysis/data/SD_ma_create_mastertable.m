...
for i = 1:length(study_list)
    ...
    tbl.stimtype = strcmp(tbl.stimulus, 'Orientation');
...
%% Table for Excel Description of Datasets
datasets     = unique(master.code);  % Get unique dataset codes
...
for i = 1:n_datasets
    tbl          = master(strcmp(master.code, datasets{i}),:);  % Subset data for the current dataset
    
    % Prepare report row with dataset info
    [ID, Study, Experiment, Condition, Stimulus, N, Trials] = deal(...
        {tbl.code{1}(1:2)}, tbl.study(1), tbl.experiment(1), tbl.cond(1), tbl.stimulus(1), ...
        numel(unique(tbl.obsid)), nnz(~isnan(tbl.(variable))));
    
    % Append report row to report table
    report_i     = table(ID, Study, Experiment, Condition, Stimulus, N, Trials);
...
cd(currentDir)
cd('docs')
writetable(report_table,'SD_ma_datasets_list.csv','Delimiter',';')
cd(currentDir)
cd('datasets')
