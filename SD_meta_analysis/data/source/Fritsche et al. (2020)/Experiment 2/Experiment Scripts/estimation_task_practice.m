...
for iBlock = 1:nBlocks
    ...
    estimation_block_practice(cfg, Gparams);
...
for iBlock = 1:nBlocks
    ...
    tmp = load(['..\Data\S' num2str(Gparams.subject_number) '\data_S' num2str(Gparams.subject_number) '_Practice_Estimation_Block_' num2str(iBlock) '.mat']);
    data_mat = cell2mat(tmp.data);
    errors = mod(data_mat(:,4) - data_mat(:,5) + 90, 180) - 90;
    mean_error = sqrt(mean(errors.^2));

    disp(['Block ' num2str(iBlock) ' - Mean Error: ' num2str(round(mean_error*100)/100) ' deg.']);
