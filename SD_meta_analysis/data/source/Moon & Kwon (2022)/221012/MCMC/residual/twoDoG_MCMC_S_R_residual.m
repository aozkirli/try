...
for k = 1:2
    ...
    for s = subjs
        switch k
            case 1
                load par_R.mat
                dataIndv{s}(:,4) = bound_180( dataIndv{s}(:,4) - DoG( dataIndv{s}(:,7), deg2rad(par.modeIndv(s,1)), deg2rad(par.modeIndv(s,2)) ) );
            case 2
                load par_S.mat
                dataIndv{s}(:,4) = bound_180( dataIndv{s}(:,4) - DoG( dataIndv{s}(:,6), deg2rad(par.modeIndv(s,1)), deg2rad(par.modeIndv(s,2)) ) );
        end
    end
...
switch k
    case 1
        output_file = strcat('DoG_residual_S_r',num2str(repeat_number),'.mat');
    case 2
        output_file = strcat('DoG_residual_R_r',num2str(repeat_number),'.mat');
end
